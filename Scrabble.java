...
public class Scrabble {
    ...
    // Populates the DICTIONARY array with words from the dictionary file
    public static void init() {
        In in = new In(WORDS_FILE);  // In object to read from dictionary file
        System.out.println("Loading word list from file...");
        NUM_OF_WORDS = 0;
        while (!in.isEmpty()) {
            DICTIONARY[NUM_OF_WORDS++] = in.readString().toLowerCase();  // Reading words
        }
        System.out.println(NUM_OF_WORDS + " words loaded.");
    }
    ...
    public static int wordScore(String word) {
        ...
        for (int i = 0; i < word.length(); i++) {
            ...
            if (letter >= 'a' && letter <= 'z') {
                ...
                // Handle invalid characters in the word
                System.out.println("Invalid character in word: " + word);
                return 0;  // Invalid word score
            }
        }
        score *= word.length();
        ...
        // Check if the word contains the sequence "runi" and add bonus points
        if (word.indexOf('r') != -1 && word.indexOf('u') != -1 && word.indexOf('n') != -1 && word.indexOf('i') != -1) {
            score += 1000;
        }
    ...
    // Creates a random hand of letters
    public static String createHand() {
        String availableLetters = "abcdefghijklmnopqrstuvwxyz";
        String hand = "";
    ...
    // Runs a single hand in the Scrabble game
    public static void playHand(String hand) {
        In in = new In();  // Create an In object to read input
        int score = 0;
        String[] playedWords = new String[100]; 
        int playedWordsCount = 0;
        ...
        while (hand.length() > 0) {
            System.out.println("Current Hand: " + formatHand(hand));
            System.out.println("Enter a word, or '.' to finish playing this hand:");
            String input = in.readString();  // Read user input
    ...
    // Format the hand to display it nicely
    public static String formatHand(String hand) {
        return hand.replace("", " ").trim(); 
    }

    // Check if the word can be formed from the given hand
    public static boolean canFormWordFromHand(String hand, String word) {
        String tempHand = hand;
        for (char letter : word.toCharArray()) {
            int index = tempHand.indexOf(letter);
    ...
    // Remove letters from the hand after the word is played
    public static String removeLettersFromHand(String hand, String word) {
        for (char letter : word.toCharArray()) {
            hand = hand.replaceFirst(String.valueOf(letter), "");  
        }
